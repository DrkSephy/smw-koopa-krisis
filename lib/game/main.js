...
MyGame = ig.Game.extend({
    ...
    bonusRequireLives: 10,

    levelVariables: [ LevelTutorial, LevelBest_overworld, LevelBoss1lair, LevelSnowy],

    levelMusic: ['smw_remix', 'map_theme', 'epic_boss', 'romancing_saga_last'],
        ...
        if(!this.showStats && !this.showEnding){
            ...
            if(ig.input.pressed('continue')) {
                ...
                if(this.myDirector.currentLevel !== this.finalStage && !this.finalStageEnabled) {
                    ...
                    if(this.lives < this.bonusRequireLives || this.finalStageEnabled) {
    ...
    toggleEnding: function(levelExit){
    ...
    draw: function() {
        ...
        } else if(this.showEnding) {
            this.statMatte.draw(0,0);
            var x = ig.system.width/2;
            var y = ig.system.height/2 - 20;
            this.statText.draw('Congratulations! You finished the game!', x, y - 30, ig.Font.ALIGN.CENTER);
            this.statText.draw('Thank you for playing!', x, y - 10, ig.Font.ALIGN.CENTER);
            this.statText.draw('We hope you enjoyed playing this game', x, y + 10, ig.Font.ALIGN.CENTER);
            this.statText.draw('as much as it was for us to make it.', x, y + 20, ig.Font.ALIGN.CENTER);
            
            if(this.lives >= this.bonusRequireLives) {
                this.statText.draw('But wait, we\'re not done yet!', x, y + 50, ig.Font.ALIGN.CENTER);
                this.statText.draw('You finished the game with all these lives...', x, y + 60, ig.Font.ALIGN.CENTER);
                this.statText.draw('Why not burn them off in our final FINAL GAUNTLET LEVEL!', x, y + 70, ig.Font.ALIGN.CENTER);
                this.statText.draw('Put your skills to the test! Prepare yourself... and good luck!', x, y + 80, ig.Font.ALIGN.CENTER);
                this.statText.draw('Press the Spacebar Continue', x, y + 110, ig.Font.ALIGN.CENTER);
            }
        }
    } // End draw
});
