...
EntityFlying_red_koopa = ig.Entity.extend({
    ...
    update: function() {
        ...
        // Enemy is within detection threshold (rectangular field-of-view) and not stalking player
        if(this.stalking == false && Math.abs(target_x - this.pos.x) <= 100 && Math.abs(target_y - this.pos.y) <= 32) {
            if(this.flip == true) { // Enemy is facing left
                if(target_x < this.pos.x) // Player is left of enemy
                    this.stalking = true;
            }
            else if(this.flip == false) { // Enemy is facing right
                if(target_x > this.pos.x) // Player is right of enemy
                    this.stalking = true;
            }
        // Enemy has detected player; Begin stalking behavior; Extend detection/stalking threshold
        } else if(this.stalking == true && Math.abs(target_x - this.pos.x) <= 150 && Math.abs(target_y - this.pos.y) <= 64) {
            if(this.flip == true) { // Enemy is facing left
                if(target_x > this.pos.x) { // Player is right of enemy
                    this.flip = false;
