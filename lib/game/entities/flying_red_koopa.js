...
EntityFlying_red_koopa = ig.Entity.extend({
    ...
    update: function() {
        ...
        if(this.distanceTo(player) < 100 && player.standing) {
            var target = 0;
            target = player.pos.x;
        }
        
        if(target < this.pos.x) { // Player is to the left of enemy
            this.flip = true;
            this.vel.x = this.speed * xdir;
        } else if(target > this.pos.x) { // Player is to the right of enemy
            this.flip = false;
            this.vel.x = this.speed * xdir;
        } else { // Player is on top of enemy
            this.vel.x = this.speed * xdir;
        }
