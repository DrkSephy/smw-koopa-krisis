        ...
        update: function() {
            ...
            if(this.ai_state != 3) {
                if(this.health <= this.ai_state_health.annoyed && this.health > this.ai_state_health.angry && this.ai_state == 0) {
                    ...
                    this.projectile_speed = this.speed * 2; // Increase projectile speed relative to movement (so player doesn't get trapped between fast enemy and slow projectile)
                    this.ai_state = 1;
                    ...
                    this.projectile_speed = this.speed * 1.7143; // Increase projectile speed relative to movement (so player doesn't get trapped between fast enemy and slow projectile)
                    this.ai_state = 2;
        ...
        collideWith: function(other, axis) {
            ...
            if(other instanceof EntityPlayer) {
                if(this.grace_time < this.graceTimer.delta()) {
                    ...
                    // Player is killed --> Exit shell state, redetect player, and stop chasing
                    if(other.health <= 0) {
