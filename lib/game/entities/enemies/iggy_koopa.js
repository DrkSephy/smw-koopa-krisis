...
.defines(function() {
    EntityIggy_koopa = EntityBasic_ai.extend({
        ...
        // AI state properties
        grace_time: 3, // Grace (non-agression) time to allow player a chance to escape if enemy is camping player respawn zone on player death
                ...
                if(my_player != null && this.grace_time < this.graceTimer.delta()) {
                    ...
                    // Enemy has detected player --> Begin chasing behavior; Extend detection/chasing range
                        ...
                        if(!setAtkTime) {
                            ...
                            var atk_dur = Math.random() * this.attack_interval.max + this.attack_interval.min;
                            setAtkTime = true;
                        // Attack procedure
