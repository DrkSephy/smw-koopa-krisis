...
7. Enemy will jump while moving.
8. Enemy will jump random heights at random intervals.
   a. Enemy may jump when firing its attack causing attack projectiles to be
      launched at jump height.
   b. See #5.

----- Protection/Shell State AI -----
9. If the player jumps on enemy when enemy is NOT in shell state, enemy will
...
10. Enemy has 3 sets of attack AIs: default, annoyed, and angry AI states.
...
----- Anti-camping AI -----
11. Enemy has anti-camping AIs. (camping = tactic where player obtains static
...
----- Abuse protection -----
13. Enemy cannot be abused from non-player damage projectiles as much.
    a. Damage from non-player projectiles that hits enemy is nerfed to 1 damage.
14. See #9b.

----- Game/Level controllers -----
15. Defeating enemy will automatically end level.
        ...
        update: function() {
            ...
            if(!inShell) {
                ...
                if(my_player != null) {
                    ...
                    if((!icu && Math.abs(target_x - this.pos.x) <= this.icu_check_dist.x && Math.abs(target_y - this.pos.y) <= this.icu_check_dist.y) || this.health < provoke_health) {
            ...
            // Enemy is in shell state
                ...
                // In panick mode and player is in range --> Start shooting projectiles randomly
                    ...
                    if(this.shootTimer.delta() > this.attack_interval.max * (this.health + this.ai_state_health.panick/4) / max_health) {
