...
.defines(function() {
    EntityGameplayer = EntityPlayer.extend({
        ...
        init: function(x, y, settings) {
            ...
            this.addAnim('hammer_climb_idle', 1, [12]);
            this.addAnim('hammer_climb', 0.4, [12,13]);
            ...
            this.addAnim('fire_climb_idle', 1, [19]);
            this.addAnim('fire_climb', 0.4, [19,20]);
        ...
        update: function() {
            ...
            // Add shooting logic
            if(ig.main.activePower != null) {
                if(this.lifetime <= 30)
                    this.lifetime +=1;
                else {
                    if(ig.input.pressed('shoot')) {
                        ig.game.spawnEntity(ig.main.activePower, this.pos.x, this.pos.y, {flip:!this.flip});
            ...
            if(ig.main.activePower == "EntityHammer") {
                ...
                else if(ig.input.state('shoot') && ig.main.activePower != null)
        ...
        receiveDamage: function(amount, from) {
            if(this.invincible)
                return;
            // Only lose powerup if player is damaged, not healed
            if(amount > 0 && ig.main.activePower != null) {
                this.losePowerSFX.volume = 0.5;
                this.losePowerSFX.play();
                ig.main.activePower = null;
            }
            //console.log(amount); //10
            //console.log(amount < 0); //true
            //console.log(this.health - amount)
            //console.log(this.max_health_player);
            // Prevent health restoration from exceeding maximum health
            if(amount < 0 && (this.max_health_player < (this.health - amount))){
                amount =  this.health - this.max_health_player;
            }
        ...
        draw: function() {
            ...
            if(ig.main.activePower != null) {
                if(ig.main.activePower == "EntityFireball")
