...
.defines(function() {
    EntityPipe = ig.Entity.extend({
        ...
        check: function(other) {
            ...
            // The +8 offset is to provide leeway in case collision occurs before player's position is updated (which attempts to prevent player randomly sliding off pipes)
            if(other.pos.y <= this.pos.y - other.size.y + 8 && !this.usingPipe) {
            ...
            // Player is using pipe
                ...
                // Entity is player; handle pipe usage
                if(other instanceof EntityGameplayer) {
                    ...
                    if(other.pos.y > this.pos.y) {
                        ...
                        // Prevent constant pipe usage in case of multiple pipes
                        this.usingPipe = false;
                    }
                // Entity is not player; handle other entity's repositioning
                } else {
                    other.flip = !other.flip;
